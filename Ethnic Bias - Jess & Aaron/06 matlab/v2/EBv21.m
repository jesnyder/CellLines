...
f1 = figure; s1=3; s2=3; si=0; y = zeros(max(CL(:,2)),1);
%Cell Line Timeline
...
%Patent 
si=si+1; subplot(s1, s2, si); hold on; Ytemp=YPaC(YPaC(:,2)==j,:); Ytemp=Ytemp(Ytemp(:,1)==2,:);
Ytemp = [0 Ytemp(1,2) (Ytemp(:,3:end))]; Ytemp = Ytemp(:,1:end-1); Y1P(j-min(CL(:,4))+1,:) = Ytemp;
i=1; while(i<=max(CL(:,2))); sc1 = scatter(Y1P(:,2),Y1P(:,i+2),5*R); sc1.MarkerFaceColor=c2(i,:); sc1.MarkerEdgeColor=c3(i,:);i=i+1;  end; 
ylabel({'# of Patents'}); ylim([0 5E5]); set(gca,'YTick',[1E0 5E0 1E1 5E1 1E2 5E2 1E3 5E3 1E4 5E4 1E5 5E5 1E6]); set(gca,'yscale','log');
xlabel({'Year'}); xlim([1950 2020]); set(gca,'XTick',[0:10:3000]);
title({ num2str(sum(Y1P(end,3:end))), 'US Patents','citing Breast Cell Lines'}); box on; set(gca,'fontsize',R); box on; hold off;

%Pie Chart for Patents by Demographic
