...
f1 = figure; s1=3; s2=6; si=0; y = zeros(max(CL(:,2)),1);
%Death Count Totals
...
i=1; while(i<=4); b1(:,i).FaceColor = c2(i,:); b1(:,i).EdgeColor = c2(i,:); i=i+1; end; 
ylabel({'Year'}); ylim([1998 2014]); set(gca,'YTick',[1998:4:2020]); 
xlabel({'Proportions'}); hold off; xlim([0 1.01]); set(gca,'XTick',[0:.1:1]);  
title({'Demographic Proportions', 'in the Death Count'}); box on; hold off; 

%Pie Chart for CDC
Ytemp=YDC(YDC(:,1)==z,:); Ytemp=Ytemp(Ytemp(:,2)==j,:); 
si=si+1; subplot(s1, s2, si);  set(gca,'fontsize',R); Z=[]; Z=Ytemp(:,3:end);
p1 = pie(Z);


    hp = findobj(p1, 'Type', 'patch');  i=1; k=1; 
    while(i<=length(hp)); if(Z(k)>0); set(hp(i), 'FaceColor', c2(k,:)); i=i+1; end; k=k+1; end;
title({num2str(j), 'Death Proportion'}); 
...
%Pie Chart for Publications by Demographic
Ytemp=YPaC(YPaC(:,2)==j,:); Ytemp=Ytemp(Ytemp(:,1)==2,:); Ytemp = [0 Ytemp(1,2) (Ytemp(:,3:end))]; Ytemp = Ytemp(:,1:end-1); Y1P(j-min(CL(:,4))+1,:) = Ytemp;
si=si+1; subplot(s1, s2, si);  Z=[]; set(gca,'fontsize',R); Z = max(Y1P(:,3:end)); 
if Z==0; p1 = pie([1], ({' '})); hp = findobj(p1, 'Type', 'patch'); set(hp(1), 'FaceColor', 'w');
else p1 = pie(Z, 0*ones(5,1), labels); hp = findobj(p1, 'Type', 'patch');  i=1; k=1; while(i<=length(hp)); if(Z(k)>0); set(hp(i), 'FaceColor', c2(k,:)); i=i+1; end; k=k+1; end; end;
title({num2str(j), 'Patent Proportion'}); 
...
xlabel({'Bias'}); xlim(.14*[-1 1]); set(gca,'XTick',[-1:.02:1]); 
...
YPaC1=YPaCn(YPaCn(:,1)==z,:);  YDC1=YDCn(YDCn(:,1)==z,:); Y = [YPaC1(:,1:2) YPaC1(:,3:end-1)-YDC1(:,3:end)]; Y=Y(Y(:,2)<=j,:);
b1 = barh(Y(:,2)-.4,Y(:,3:end),'BarWidth',w); view([90 270]);
i=1; while(i<=4); b1(:,i).FaceColor = c2(i,:); b1(:,i).EdgeColor = c2(i,:); i=i+1; end; 
ylabel({'Year'}); ylim([1998 2014]); set(gca,'YTick',[1950:2:2020]); 
xlabel({'Bias'}); xlim(.14*[-1 1]); set(gca,'XTick',[-1:.02:1]); 
