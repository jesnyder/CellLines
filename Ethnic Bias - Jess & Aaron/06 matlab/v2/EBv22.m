...
%title({ num2str(length(PaCo)), 'Cell Lines cited more than' , num2str(yy), 'times in US Patents'}); 
ylim([yy 1E5]); set(gca,'YTick',[1E0 1E1 1E2 1E3 1E4 1E5 1E6]);
xlim([0 length(PaCo)*1.2]); 
xlabel({ num2str(length(PaCo)), 'Cell Lines'});
ylabel({num2str(yy), '<Citations', 'in US Patents'});
yy=10*yy; i=i+1; end; hold off;

% Pie chart for patent representation
yy=1; while(si<2*s2); C1=CL(CL(:,end-1)>=yy,:); C1=C1(C1(:,1)==z,:); 
...
else p1 = pie(Z, 0*ones(5,1), labelsb); hp = findobj(p1, 'Type', 'patch');  i=1; k=1; while(i<=length(hp)); if(Z(k)>0); set(hp(i), 'FaceColor', c2(k,:)); i=i+1; end; k=k+1; end; end;
...
Ct=transpose([1:1:length(C1)]); C1 = [Ct C1];
yy=1; i=2*s2; while(i<3*s2); PaCo=C1(C1(:,4)>yy,:); 
si=si+1; subplot(s1, s2, si); hold on; set(gca,'yscale','log'); box on; 
k=1; while(k<=max(C1(:,2))); P1=PaCo(PaCo(:,2)==k,:); b1 = scatter(P1(:,1),P1(:,4), 'MarkerEdgeColor', c2(k,:)); k=k+1; end; 
%title({ num2str(length(PaCo)), 'Cell Lines cited more than' , num2str(yy), 'times in Publications'}); 
xlabel({ num2str(length(PaCo)), 'Cell Lines'});
ylabel({num2str(yy), '<Citations','in Publications'});
ylim([yy 1E5]); set(gca,'YTick',[1E0 1E1 1E2 1E3 1E4 1E5 1E6]);
xlim([0 length(PaCo)*1.2]); 
yy=10*yy; i=i+1; end; hold off;

% Pie chart for patent representation
yy=1; si=3*s2; while(si<s2*s1); C1=CL(CL(:,end)>=yy,:); C1=C1(C1(:,1)==z,:); 
Z=[]; k=1; while(k<=5); Ca=C1(C1(:,2)==k,:); Z(k)=length(Ca(:,1)); k=k+1; end; 
si=si+1; subplot(s1, s2, si); set(gca,'fontsize',R); 
if Z==0; p1 = pie([1], ({' '})); hp = findobj(p1, 'Type', 'patch'); set(hp(1), 'FaceColor', 'w');
else p1 = pie(Z, 0*ones(5,1), labelsb); hp = findobj(p1, 'Type', 'patch');  i=1; k=1; while(i<=length(hp)); if(Z(k)>0); set(hp(i), 'FaceColor', c2(k,:)); i=i+1; end; k=k+1; end; end;
