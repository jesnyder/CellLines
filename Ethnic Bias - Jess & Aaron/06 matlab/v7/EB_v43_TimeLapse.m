...
CL = csvread('CellLineSum.csv',1,1); CL=CL(CL(:,4)>0,:); CL=CL(CL(:,6)<1,:); CL=CL(:,1:5); CL = sortrows(CL,4); % Read Cell Line Descriptions. Remove all unknown years, remove any cell lines known to have contaminants, and sort by year the cell line was introduced 
...
PaC = csvread('PatCount.csv',1,0); PaC = sortrows(PaC,4); % Read Cell Line Descriptions. Remove all unknown years and sort by year the cell line was introduced 
