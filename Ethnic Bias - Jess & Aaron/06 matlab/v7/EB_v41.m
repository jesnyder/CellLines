...
a=4; while(a<=6); CDCt=CDC(CDC(:,1)==a,:); CDCt=CDCt(CDCt(:,2)==t,:); i=1; while(i<=length(CDCt(:,1))) CDCs(i,1)=CDCt(i,3); if(CDCt(i,1)==4); CDCs(i,2)=CDCt(i,end);  elseif(CDCt(i,1)==5); CDCs(i,3)=CDCt(i,end); elseif(CDCt(i,1)==6) CDCs(i,4)=CDCt(i,end); end; i=i+1; end; a=a+1; end; 
...
% Download publication count from https://docs.google.com/spreadsheets/d/1PX1VQP5OoNXjQJK9c-Jpbic9BL3H0Af29erB8deGvUw/edit#gid=0
PaC = csvread('PatCount.csv', 1, 0); PaC=PaC(PaC(:,3)==4,:); PaC=[PaC(:,1), PaC(:,4)]; % Disease specific Publication count 
...
AR =.5; size = 1200; set(f1, 'Position', [0 0 size AR*size]); savedir=strcat('Lung Figures'); mkdir(savedir); newname=[savedir '/' ,'LungComposite.jpg']; 
