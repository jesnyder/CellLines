...
YIC=[]; YICn=[]; m=1; while(m<=length(CDC(:,1))); B=CDC; B=B(B(:,1)==m,:); B=[ B(:,1:2) B(:,14:17) zeros(length(B(:,1)),1)]; Bn=B;
...
f1 = figure; s1=1; s2=3; si=0; 

si = si+1; subplot(s1, s2, si);
hold on; B=YIC(YIC(:,1)==2,:);
 scatter(YIC(:,2),YIC(:,2+1)); 
ylabel({'Incidence Count'}); 
xlabel({'Year'}); xlim([1990 2012]); set(gca,'XTick',[1950:10:2020]); 
box on;  hold off;

si = si+1; subplot(s1, s2, si);
hold on; B=YIC(YIC(:,1)==2,:);
scatter(YDC(:,2),YDC(:,2+1)); 
ylabel({'Incidence Count'}); 
xlabel({'Year'}); xlim([1990 2012]); set(gca,'XTick',[1950:10:2020]); 
box on;  hold off;
