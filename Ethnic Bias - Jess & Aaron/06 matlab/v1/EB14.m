clc; clearvars; R=15; w=1; c1 = 0*ones(5,3); c2=[1 0 0 ; 0 1 0; 0 0 1; 0 1 1; .8 .8 .8]; c3=.7*c2; 
...
j=1960; 
while (j<2016)
    si=si+1; subplot(s1, s2, si); 
    Z=[]; Y1=CL(CL(:,4)<=j,:); 
    i=1; while i<=max(CL(:,2)); Ya=Y1(Y1(:,2)==i,:); Z(i)=length(Ya); i=i+1; end;  p1 = pie(Z, .02*ones(5,1), labels'); 
    hp = findobj(p1, 'Type', 'patch'); 
    i=1; k=1; while(i<=length(hp)); 
        if(Z(k)>0); set(hp(i), 'FaceColor', c2(k,:)); i=i+1; end; k=k+1; end; 
    title({'1951-' num2str(j)}); 
j=j+10; 
if(j>2010); j=2016; end;
if(si==6) si=si+3; end;
if(si==12) si=si+3; end;
end;
