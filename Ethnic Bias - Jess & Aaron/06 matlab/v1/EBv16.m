...
%Cell Line Timeline
si=si+1; subplot(s1, s2, si); hold on; Y1=YCL(YCL(:,1)==2,:); Y1=Y1(Y1(:,2)<=j,:);
i=1; while i<=max(CL(:,2)); sc1 = scatter(Y1(:,2),Y1(:,i+2),5*R); sc1.MarkerFaceColor=c2(i,:);sc1.MarkerEdgeColor=c3(i,:); i=i+1;  end; 
ylabel({'Demographic'}); ylim([.5 40]); set(gca,'YTick',[0:5:200]); %set(gca,'YTickLabel',labels);
xlabel({'Year Cell Line was Established'}); xlim([1950 2020]); set(gca,'XTick',[1950:10:2020]);
if(sum(sum(Y1(:,3:end)))==0); title({ ' ', 'Human Cell Lines','from Breast Biopsies' }); 
else title({ ' ', 'Human Cell Lines','from Breast Biopsies' }); 
box on; set(gca,'fontsize',R); hold off; 

%Pie Chart for Cell Lines by Demographic
si=si+1; subplot(s1, s2, si);  set(gca,'fontsize',R); Z = Y1(end,3:end-1); 
...
Y=YCLn(YCLn(:,1)==2,:); Y=Y(Y(:,2)<=j,:); 
b1 = barh(Y(:,2),Y(:,3:end),'Stacked','BarWidth',w); view([90 270]);
i=1; while(i<=5); b1(:,i).FaceColor = c2(i,:); b1(:,i).EdgeColor = [0 0 0]; i=i+1; end; 
ylabel({'Year'}); ylim([1950 2020]); set(gca,'YTick',[1950:10:2020]); 
xlabel({'Demographic Proportions', 'in the Cell Line Catalog'}); xlim([0 1.05]);  set(gca,'XTick',[-1:.1:1]); hold off; 
...
ylabel({'Publication Count'}); ylim([0 3E4]); set(gca,'YTick',[1E0 5E0 1E1 5E1 1E2 5E2 1E3 5E3 1E4 5E4 1E5 5E5 1E6]); set(gca,'yscale','log');
xlabel({'Year'}); xlim([1950 2020]); set(gca,'XTick',[0:10:3000]);
title({ num2str(sum(Y1P(end,3:end))), 'Medline/Scopus Publications','citing Breast Cell Lines'}); box on; set(gca,'fontsize',R); hold off; 
...
i=1; while(i<=5); b1(:,i).FaceColor = c2(i,:); b1(:,i).EdgeColor = [0 0 0]; i=i+1; end; 
ylabel({'Year'}); ylim([1950 2020]); set(gca,'YTick',[1950:10:2020]); 
xlabel({'Demographic Proportions' ,'in the Publication'}); xlim([0 1.05]);  set(gca,'XTick',[-1:.1:1]); hold off; 

AR = 9/16; %9/16 is youtube aspect ratio
size = 900; set(f1, 'Position', [0 0 size AR*size]);
savedir=strcat('Cell_Line_Time_Lapse_Draft'); mkdir(savedir);
