...
Y1P = []; j=min(CL(:,4)); while(j<=2016); close all;
...
%Cell Line Timeline
si=si+1; subplot(s1, s2, si); hold on; Y1=CL(CL(:,4)<=j,:); Y1=Y1(Y1(:,3)==2,:); 
...
title({'The Human Cell Line Catalog','derived from Breast Biopsies'}); box on; set(gca,'fontsize',R); hold off; 

%Cell Line Count
si=si+1; subplot(s1, s2, si); hold on; 
i = 1; while(i<=max(CL(:,2))); Ya=Y1(Y1(:,2)==i,:); if(length(Ya)==0); else y(i)=y(i)+length(Ya(:,1)); end; i=i+1; end;
b1 = barh(y); set(b1(:,1),'facecolor','w')
ylabel({'Demographic'}); ylim([.5 5.5]); set(gca,'YTick',[1:1:6]); set(gca,'YTickLabel',labels);
xlabel({'Number of Cell Lines' }); xlim([0 50]); set(gca,'XTick',[0:10:150]);
title({ num2str(sum(y)), 'Human Cell Lines' }); box on; set(gca,'fontsize',R); hold off; 

%Pie Chart for Cell Lines by Demographic
si=si+1; subplot(s1, s2, si);  Z=[]; set(gca,'fontsize',R); 
i=1; while i<=max(CL(:,2)); Ya=Y1(Y1(:,2)==i,:); Z(i)=length(Ya); i=i+1; end;  
...
title({'Cumulative Publication Record','citing Breast Cell Lines'}); box on; set(gca,'fontsize',R); hold off; 

% Counts of Publications
si=si+1; subplot(s1, s2, si); hold on; y=max(Y1P(:,3:end)); b1 = barh(y); set(b1(:,1),'facecolor','w')
ylabel({'Demographic'}); ylim([.5 5.5]); set(gca,'YTick',[1:1:6]); set(gca,'YTickLabel',labels);
xlabel({'Number of Publications'}); xlim([0 3E4]); set(gca,'XTick',[1E0 1E1 1E2 1E3 1E4 1E5 1E6]); set(gca,'xscale','log');
title({ num2str(sum(Y1P(end,3:end))), 'Medline/Scopus Publications'}); box on; set(gca,'fontsize',R); hold off; 
